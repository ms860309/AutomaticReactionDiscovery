...
#run ard job
def run_ard():
        ard_path = os.path.join(abspath_pardir, 'ard.py')
        input_path = os.path.join(abspath_pardir, 'input')
        p = subprocess.Popen(['python', ard_path, input_path])
        p.wait()
#run gsm job
def run_GSM():
        reactions_path = os.path.join(abspath_pardir, 'reactions')
        dirs = os.listdir(reactions_path)
        gsm_path = os.path.join(abspath_pardir, '/pyGSM/examples/QChem/DE_GSM/')
        for i in dirs:
