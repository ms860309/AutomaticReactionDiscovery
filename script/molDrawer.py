...
def molDrawer():
    ...
    for i in dirs:
        dir_path = os.path.join(os.path.join('/mnt/d/reactions', i), 'ssm_product.xyz')
...
def generate_ssm_product_xyz():
    ...
    for i in reactions_path_list:
        ...
        product_xyz = os.path.join(os.path.join(target_path, 'SSM'), opt_file[-1])
        with open(product_xyz, 'r') as f:
            ...
            for i in reversed(lines):
                ...
                if len(a) == 1:
                    idx = lines.index(i)
                    break
        parent_ssm_product_path  = os.path.join(os.path.abspath(os.path.join(os.path.join(target_path, 'SSM'), '../')), 'ssm_product.xyz')
